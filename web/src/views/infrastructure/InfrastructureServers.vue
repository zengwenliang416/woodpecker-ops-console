  ...
  <Scaffold v-model:search="search">
    ...
    <section class="wp-card">
      ...
      <div class="wp-table-scroll">
        <table>
          <thead><tr><th>{{ $t('ops.server.name') }}</th><th>{{ $t('ops.server.status') }}</th><th>{{ $t('ops.server.environment') }}</th><th>CPU</th><th>{{ $t('ops.server.memory') }}</th><th>{{ $t('ops.server.disk') }}</th><th>{{ $t('ops.server.runtime') }}</th><th>{{ $t('ops.server.heartbeat') }}</th><th /></tr></thead>
          <tbody>
            <tr v-for="server in filteredServers" :key="server.id">
              ...
              <td><IconButton icon="settings" :title="$t('ops.servers.manage')" :to="`/infrastructure/servers/${server.id}`" /></td>
            </tr>
            <tr v-if="filteredServers.length === 0"><td colspan="9" class="wp-empty-state"><strong>{{ $t('ops.servers.none') }}</strong><p>{{ $t('ops.servers.none_hint') }}</p></td></tr>
          </tbody>
        </table>
      </div>
    </section>

    <div v-if="showRegister" class="fixed inset-0 z-40 flex items-center justify-center bg-black/50 p-4" @click.self="showRegister = false">
      <form class="wp-card w-full max-w-md p-5 wp-stack" @submit.prevent="registerServer">
        <h2 class="text-lg font-bold">{{ $t('ops.servers.register') }}</h2>
        <label class="field"><span class="field-label">{{ $t('ops.server.name') }}</span><input v-model="form.name" class="input" required /></label>
        <label class="field"><span class="field-label">{{ $t('ops.server.runtime') }}</span>
          <select v-model="form.runtime" class="select"><option>docker</option><option>kubernetes</option><option>systemd</option></select>
        </label>
