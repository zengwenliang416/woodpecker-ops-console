  ...
  <Scaffold>
    ...
    <section class="wp-card">
      <div class="wp-table-scroll">
        <table>
          <thead>
            <tr>
              <th>{{ $t('ops.applications.name') }}</th>
              <th>Image</th>
              <th>{{ $t('ops.applications.runtime') }}</th>
              <th>{{ $t('ops.applications.releases') }}</th>
              <th />
            </tr>
          </thead>
          <tbody>
            <tr v-for="app in appStore.applicationList" :key="app.id">
              <td>
                <router-link class="text-wp-link-100" :to="`/deployments/apps/${app.id}`">{{ app.name }}</router-link>
              </td>
            ...
            <tr v-if="appStore.applicationList.length === 0">
              <td colspan="5" class="wp-empty-state">
                <strong>{{ $t('ops.applications.none') }}</strong>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    >
      <form class="wp-card wp-stack w-full max-w-md p-5" @submit.prevent="createApp">
        ...
        <label class="field"
          ><span class="field-label">Name</span><input v-model="form.name" class="input" required
        /></label>
        <label class="field"
          ><span class="field-label">Image</span><input v-model="form.image" class="input wp-mono" required
        /></label>
        <label class="field"
        ...
        <div class="wp-cluster">
          <Button type="submit" color="green" :text="$t('ops.applications.save')" /><Button
            type="button"
            variant="ghost"
            :text="$t('cancel')"
