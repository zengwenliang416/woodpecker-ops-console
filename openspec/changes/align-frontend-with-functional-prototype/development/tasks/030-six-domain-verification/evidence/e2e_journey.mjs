...
/**
 * Task 030 e2e journey. Captures the key user journey (guest login light,
 * authenticated overview light and dark, repositories dark, deployment
 * detail with log console dark) and verifies: data-theme application, theme
 * parity (body backgrounds differ), zero page-level horizontal overflow,
 * zero console/runtime/network/HTTP failures, and local scroll containment
 * of the log console under genuine horizontal overflow.
 */
...
const mockApiUrl = process.env.TASK030_MOCK_URL ?? 'http://127.0.0.1:8282';
const productionUrl = process.env.TASK030_PRODUCTION_URL ?? 'http://127.0.0.1:8280';
...
const states = [
  { id: 'journey-login-light', theme: 'light', locale: 'zh-Hans', path: '/login', patterns: [/登录/], guest: true },
  { id: 'journey-overview-light', theme: 'light', locale: 'zh-Hans', path: '/overview', patterns: [/backend-api/, /概览/] },
  { id: 'journey-overview-dark', theme: 'dark', locale: 'zh-Hans', path: '/overview', patterns: [/backend-api/, /概览/] },
  { id: 'journey-repos-dark', theme: 'dark', locale: 'zh-Hans', path: '/repos', patterns: [/acme\/backend-api/, /acme\/web-frontend/] },
  { id: 'journey-deployment-detail-dark', theme: 'dark', locale: 'zh-Hans', path: '/deployments/142', patterns: [/DEP-142|142/, /日志|logs|pulling/i] },
];
...
async function waitForDocument(client, patterns, timeoutMs = 40_000) {
  ...
  while (Date.now() < deadline) {
    ...
    if (process.env.TASK030_DEBUG === '1') {
...
async function capture(state, remotePort) {
  ...
  const result = await client.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const logContainment = logConsole
        ? {
            clientWidth: logConsole.clientWidth,
            scrollWidth: logConsole.scrollWidth,
            overflowX: getComputedStyle(logConsole).overflowX,
            locallyScrollable:
              logConsole.scrollWidth > logConsole.clientWidth + 1 &&
              ['auto', 'scroll'].includes(getComputedStyle(logConsole).overflowX),
          }
        : null;
      return {
  ...
  return {
    schema: 'woodpecker.task030-e2e-journey.v1',
    stateId: state.id,
    runId,
    theme: state.theme,
    locale: state.locale,
...
try {
  ...
  chromeProfile = await mkdtemp(path.join(tmpdir(), 'task030-chrome-'));
  chrome = spawn(
    chromeBinary,
    [
      '--headless=new',
  ...
  for (const [route, themes] of byRoute) {
    if (themes.light && themes.dark && themes.light.bodyBackground === themes.dark.bodyBackground) {
  ...
  for (const m of measurements) {
    ...
    if (m.route === '/deployments/142' && m.logContainment && !m.logContainment.locallyScrollable) {
      failures.push(`${m.stateId}: log console not locally scrollable (${JSON.stringify(m.logContainment)})`);
    }
  ...
  await writeFile(path.join(evidenceRoot, 'e2e-journey-summary.json'), `${JSON.stringify(summary, null, 2)}\n`);
  await writeFile(
    path.join(evidenceRoot, 'e2e-journey-measurements.json'),
