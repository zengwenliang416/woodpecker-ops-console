  ...
  <Scaffold>
    ...
    <section class="wp-card">
      <div class="wp-table-scroll">
        <table>
          <thead><tr><th>ID</th><th>{{ $t('ops.deployment.application') }}</th><th>{{ $t('ops.deployment.environment') }}</th><th>{{ $t('ops.deployment.release') }}</th><th>{{ $t('ops.deployment.triggered_by') }}</th><th /></tr></thead>
          <tbody>
            ...
            <tr v-if="store.pendingApprovals.length === 0"><td colspan="6" class="wp-empty-state"><strong>{{ $t('ops.approvals.none') }}</strong></td></tr>
          </tbody>
        </table>
      </div>
    </section>
