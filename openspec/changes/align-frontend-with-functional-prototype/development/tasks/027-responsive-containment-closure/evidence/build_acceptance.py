#!/usr/bin/env python3
"""Build the Task 027 acceptance-evidence JSON at the closure HEAD."""
import hashlib
import json
import subprocess
...
TASK = f"{CHANGE}/development/tasks/027-responsive-containment-closure"
...
summary = json.load(open(f"{ROOT}/{EVIDENCE}/browser-replay-summary.json"))
manifest = json.load(open(f"{ROOT}/{EVIDENCE}/evidence-manifest.json"))
log_path = f"{CHANGE}/development/evidence/038-027-responsive-containment-closure.log"
...
acceptance = {
    ...
    "task_id": "027-responsive-containment-closure",
    "generated_at": now,
    "recorded_at": now,
    "status": "approved",
    ...
    "test_runs": [
        {
            "id": receipt_id,
            "command": "node openspec/changes/align-frontend-with-functional-prototype/development/tasks/027-responsive-containment-closure/evidence/validate_task.mjs",
            "assertion_ids": ["A2", "A3"],
            "recorded_at": now,
            "validation_receipt_sha256": sha256(f"{ROOT}/{EVIDENCE}/evidence-manifest.json"),
            "evidence_log": {
                "path": log_path,
                "sha256": log_sha,
                "size": log_size,
