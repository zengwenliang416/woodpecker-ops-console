<template>
  ...
  <Scaffold>
    ...
    <div class="mb-4"><DeploymentNav /></div>
    <div v-if="policies" class="wp-grid wp-grid-sidebar">
      <section class="wp-card">
        ...
        <div class="wp-card-body wp-grid wp-grid-2">
          ...
          <div>{{ policies.health_check_interval }}s</div>
          ...
          <div>{{ policies.auto_rollback ? '✓' : '—' }}</div>
        </div>
      </section>
      <section class="wp-card">
        <div class="wp-card-header">
          <h2>{{ $t('ops.policies.strategies') }}</h2>
        </div>
        <div class="wp-card-body wp-stack">
          <div class="wp-split">
            <span>{{ $t('ops.deployment.strategy_single') }}</span
            ><span class="wp-muted">1 × 1</span>
          </div>
          <div class="wp-split">
            <span>{{ $t('ops.deployment.strategy_rolling') }}</span
            ><span class="wp-muted">batch {{ policies.batch_size_default }}</span>
          </div>
          <div class="wp-split">
            <span>{{ $t('ops.deployment.strategy_all_at_once') }}</span
            ><span class="wp-muted">N × N</span>
          </div>
        </div>
      </section>
    </div>
