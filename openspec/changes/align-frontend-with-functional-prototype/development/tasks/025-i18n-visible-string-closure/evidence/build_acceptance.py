#!/usr/bin/env python3
"""Build the Task 025 acceptance-evidence JSON at the closure HEAD."""
import hashlib
import json
import subprocess
...
TASK = f"{CHANGE}/development/tasks/025-i18n-visible-string-closure"
...
log_path = f"{CHANGE}/development/evidence/038-025-i18n-visible-string-closure.log"
...
acceptance = {
    ...
    "task_id": "025-i18n-visible-string-closure",
    "generated_at": now,
    "recorded_at": now,
    "status": "approved",
    ...
    "test_runs": [
        {
            "id": receipt_id,
            "command": "npx vitest run src/i18nVisibleStrings.test.ts && npx vitest run",
            "assertion_ids": ["A2", "A3"],
            "recorded_at": now,
