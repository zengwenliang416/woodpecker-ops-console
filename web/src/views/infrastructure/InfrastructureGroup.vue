  ...
  <Scaffold :go-back="() => router.push('/infrastructure/groups')">
    ...
    <div v-if="group" class="wp-grid wp-grid-sidebar">
      <section class="wp-card">
        <div class="wp-card-header"><h2>{{ $t('ops.groups.details') }}</h2></div>
        <div class="wp-card-body wp-grid wp-grid-2">
          <div class="wp-muted">{{ $t('ops.groups.description') }}</div><div>{{ group.description || '—' }}</div>
          <div class="wp-muted">{{ $t('ops.deployment.environment') }}</div><div>{{ envName(group.environment_id) }}</div>
          <div class="wp-muted">{{ $t('ops.deployment.strategy') }}</div><div>{{ group.strategy }} · batch {{ group.batch_size }}</div>
          <div class="wp-muted">Health</div><div>{{ group.health_path || '—' }} :{{ group.port || '—' }}</div>
        </div>
      </section>
      <section class="wp-card">
        <div class="wp-card-header"><h2>{{ $t('ops.groups.servers') }}</h2></div>
        <div class="wp-table-scroll">
          <table>
            <thead><tr><th>{{ $t('ops.server.name') }}</th><th>{{ $t('ops.server.status') }}</th><th>CPU</th></tr></thead>
            <tbody>
