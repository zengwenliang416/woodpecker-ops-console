<template>
  ...
  <div ref="tabsRef" class="flex min-w-0 flex-auto gap-4">
    ...
    >
      <span
        class="hover:bg-wp-control-neutral-100 dark:hover:bg-wp-control-neutral-200 flex w-full min-w-20 flex-row items-center justify-center gap-2 rounded-lg px-2 py-1"
      >
    ...
    <div v-if="hiddenTabs.length" class="relative border-b-2 border-transparent py-1">
      ...
      <div
        v-if="isDropdownOpen"
        class="tabs-dropdown border-wp-background-400 dark:border-wp-background-100 bg-wp-background-100 dark:bg-wp-background-200 dark:shadow-wp-background-500 absolute z-20 mt-1 rounded-lg border shadow-lg"
      ...
      >
        ...
        >
          <span
            class="hover:bg-wp-control-neutral-100 dark:hover:bg-wp-control-neutral-200 flex w-full min-w-20 flex-row gap-2 rounded-lg px-2 py-1"
          >
            <Icon v-if="tab.icon" :name="tab.icon" :class="tab.iconClass" class="shrink-0" />
            <span>{{ tab.title }}</span>
          </span>
