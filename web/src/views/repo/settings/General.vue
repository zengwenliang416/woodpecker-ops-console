<template>
  <div class="repo-settings-page">
    <header class="repo-settings-page-header">
      <div>
        <h1>{{ $t('repo.settings.general.project') }}</h1>
        <p>{{ $t('repo.settings_surface.general.description') }}</p>
      </div>
    </header>
    ...
    <FeedbackState
      v-if="!repoSettings"
      kind="loading"
      :title="$t('repo.settings_surface.general.loading_title')"
      :description="$t('repo.settings_surface.general.loading_description')"
    />

    <form v-else class="space-y-4" @submit.prevent="saveRepoSettings">
      <RepoSettingsSection
        :title="$t('repo.settings_surface.general.pipeline_behavior_title')"
        :description="$t('repo.settings_surface.general.pipeline_behavior_description')"
      >
        <Checkbox
      ...
      <RepoSettingsSection
        :title="$t('repo.settings_surface.general.execution_title')"
        :description="$t('repo.settings_surface.general.execution_description')"
      >
      ...
      <RepoSettingsSection
        :title="$t('repo.settings_surface.general.trust_title')"
        :description="$t('repo.settings_surface.general.trust_description')"
      >
        <InputField
...
const repoSettingsOwnerId = ref<number>();
...
function loadRepoSettings() {
  repoSettingsOwnerId.value = repo.value.id;
  repoSettings.value = {
    ...
    trusted: deepClone(repo.value.trusted),
...
async function loadRepo(repoId: number, requestGeneration: number) {
...
const { doSubmit: saveRepoSettings, isLoading: isSaving } = useAsyncAction(async () => {
  if (!repoSettings.value || repoSettingsOwnerId.value !== repo.value.id) {
  ...
  await apiClient.updateRepo(repoId, deepClone(repoSettings.value));
  if (repo.value.id !== repoId || requestGeneration !== repoLifecycleGeneration) {
    return;
  }
  await loadRepo(repoId, requestGeneration);
  if (repo.value.id === repoId && requestGeneration === repoLifecycleGeneration) {
