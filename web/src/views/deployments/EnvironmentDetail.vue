  ...
  <Scaffold :go-back="() => router.push('/deployments/environments')">
    ...
    <div v-if="env" class="wp-stack">
      <div class="wp-cluster">
        <span v-if="env.protected" class="badge text-wp-state-warn-100"><Icon name="shield" />{{ $t('ops.environment.protected') }}</span>
        <span v-if="env.approval_required" class="badge text-wp-state-warn-100">{{ $t('ops.environment.approval_required') }} ({{ env.minimum_approvers }})</span>
        <span v-if="env.auto_rollback" class="badge text-wp-state-ok-100">{{ $t('ops.environment.auto_rollback') }}</span>
      </div>
      <div class="wp-grid wp-grid-sidebar">
        <section class="wp-card">
          <div class="wp-card-header"><h2>{{ $t('ops.environment.details') }}</h2></div>
          <div class="wp-card-body wp-grid wp-grid-2">
            <div class="wp-muted">{{ $t('ops.environment.deploy_window') }}</div><div>{{ env.deploy_window || '—' }}</div>
            <div class="wp-muted">Domain</div><div>{{ env.domain || '—' }}</div>
            <div class="wp-muted">{{ $t('ops.environment.minimum_approvers') }}</div><div>{{ env.minimum_approvers }}</div>
          </div>
        </section>
        <section class="wp-card">
...
onMounted(() => Promise.all([appStore.loadEnvironments(), serverStore.loadGroups()]));
</script>
